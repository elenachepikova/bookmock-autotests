...
class Assertions(BaseActions):
    ...
    def assert_element_absense(self, selector):
        ...
        assert len(elements) == 0, "Unexpected element(s) present on page"

    @allure.step('Assert page title and URL')
    def assert_page_title_and_url(self, title, url):
        if title is not None:
            assert self.driver.title == title, (f"Title should be {title}, "
                                                f"but is {self.driver.title}")
        if url is not None:
            assert self.driver.current_url == url, (f"Url should be {url}, "
                                                    f"but is {self.driver.current_url}")

    @allure.step('Assert {selector} field contains text {text}')
    def assert_text(self, selector, text):
        element = self.wait_for_element(selector)
        assert element.text == text, (f'Expected text: {text}, '
