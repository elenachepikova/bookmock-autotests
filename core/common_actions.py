...
class CommonActions(Assertions):
    ...
    def assert_first_book_title(self, expected_title):
        ...
        ), f"Expected product title - {expected_title}, actual - {actual_title}"
    ...
    @allure.step(
        "Assert valid message if displayed if no results matches search criteria"
    )
    ...
    def assert_products_count(self, value):
        assert self.count_elements(self.PRODUCTS) == value, (
            f"Expected products count = {value}, "
